...
if not os.path.exists("submissions"):
    ...
    nameOfZip = input(
        "What is the name of your compressed submissions file? > "
    )
...
for file in sorted(os.listdir("submissions")):
    if file.endswith(".tar.gz"):
        ...
        if (
            nameOfAssignment + ".h" in folder
            or nameOfAssignment.capitalize() + ".h"
            in folder
        ):
        ...
        if (
            nameOfAssignment + ".h" not in os.listdir()
            and nameOfAssignment.capitalize() + ".h"
            not in os.listdir()
        ):
            here = glob.glob("**/*.h", recursive=True)[0]
            os.chdir("..")
            if (
                nameOfAssignment + ".h" in here
                or nameOfAssignment.capitalize() + ".h"
                in here
            ):
                here = glob.glob("**/*.h", recursive=True)[
                    0
                ]
                dirs = here.split("/")
                os.makedirs(dirs[-2])
                for fileThatNeedMoving in os.listdir(
                    "/".join(dirs[:-1])
                ):
                    os.rename(
                        "/".join(dirs[:-1])
        ...
        for files in os.listdir():
            if (
                files != nameOfAssignment + ".cpp"
                and files != nameOfAssignment + ".h"
                and files
                != nameOfAssignment.capitalize() + ".cpp"
                and files
                != nameOfAssignment.capitalize() + ".h"
            ):
                os.remove(files)
        ...
        gccCompile = Popen(
            cppFiles, stderr=STDOUT, stdout=sys.stdout
        )
        gccOutput, gccStatusCode = (
            gccCompile.communicate()[0],
            gccCompile.returncode,
        )
        if gccStatusCode != 0:
            # print(gccOutput)
            print(
                "An error occured during compiling the code!"
            )
            os.chdir("..")
            shutil.rmtree(folder)
            os.chdir("..")
        ...
        if executeStatusCode != 0:
            if executeStatusCode == -11:
                ...
                print(
                    "Error has occured", executeStatusCode
                )
                outputFile.write(
                    "Error " + str(executeStatusCode) + "\n"
                )
        else:
